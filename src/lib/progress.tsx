...
export function ProgressProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("user_progress")
        .select("completed, xp, streak, last_day, badges")
        .eq("user_id", user.id)
        .maybeSingle();
      if (cancelled) return;
      if (error) {
  ...
  }, [progress, user, mounted, syncState]);
  ...
  return (
    <ProgressContext.Provider
      value={{ progress, isCompleted, isUnlocked, completeLesson, reset, cloudEnabled: !!user, syncState }}
    >
      {children}
    </ProgressContext.Provider>
  );
