      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <Link to="/profile" aria-label="حسابي" className="grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-card">
                {avatarUrl(user) ? (
                  <img src={avatarUrl(user)!} alt={displayName(user)} className="h-9 w-9 object-cover" loading="lazy" />
                ) : (
                  <span className="text-xs font-extrabold">{displayName(user).slice(0, 1).toUpperCase()}</span>
                )}
              </Link>
            ) : (
              <Link
